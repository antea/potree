...
export var Globals = {
    ...
    resourcePath: "",
    pointLoadLimit: 0,
    updatePointClouds: function (pointclouds, camera, renderer) {
        ...
        let result = Globals.updateVisibility(pointclouds, camera, renderer);
        ...
        Globals.lru.freeMemory(Globals.pointLoadLimit);

        return result;
    },
    updateVisibilityStructures: function (pointclouds, camera, renderer) {
        let frustums = [];
        let camObjPositions = [];
    ...
    updateVisibility: function (pointclouds, camera, renderer) {
        ...
        let s = Globals.updateVisibilityStructures(pointclouds, camera, renderer);
        ...
        { // update DEM
            ...
            let candidates = pointclouds
                .filter(p => (p.generateDEM && p.dem instanceof Globals.DEM));
...
if (document.currentScript && document.currentScript.src) {
    Globals.scriptPath = new URL(document.currentScript.src + '/..').href;
    if (Globals.scriptPath.slice(-1) === '/') {
        Globals.scriptPath = Globals.scriptPath.slice(0, -1);
    }
} else if (import.meta) {
    Globals.scriptPath = new URL(import.meta.url + "/..").href;
    if (Globals.scriptPath.slice(-1) === '/') {
        Globals.scriptPath = Globals.scriptPath.slice(0, -1);
    }
} else {
...
Globals.resourcePath = Globals.scriptPath + '/resources';