...
export class PointCloudArena4DNode extends PointCloudTreeNode {
	...
	toTreeNode (child) {
		...
		let sceneNode = new THREE.Points(geometryNode.geometry, this.kdtree.material);
...
export class PointCloudArena4D extends PointCloudTree{
	...
	computeVisibilityTextureData(nodes){
		...
		if(Globals.measureTimings) performance.mark("computeVisibilityTextureData-start");
	...
	get progress () {
		if (this.pcoGeometry.root) {
			return Globals.numNodesLoading > 0 ? 0 : 1;
		} else {
			return 0;
		}
	}
